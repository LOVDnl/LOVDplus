...
//    ' . $sSettings
   ...
   'CREATE TABLE ' . TABLE_SCHEDULED_IMPORTS . ' (
    ...
    INDEX (genelistid),
	INDEX (created_by),
    CONSTRAINT ' . TABLE_IND2GL . '_fk_individualid FOREIGN KEY (individualid) REFERENCES ' . TABLE_INDIVIDUALS . ' (id) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT ' . TABLE_IND2GL . '_fk_genelistid FOREIGN KEY (genelistid) REFERENCES ' . TABLE_GENE_LISTS . ' (id) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT ' . TABLE_IND2GL . '_fk_created_by FOREIGN KEY (created_by) REFERENCES ' . TABLE_USERS . ' (id) ON DELETE SET NULL ON UPDATE CASCADE)
    ' . $sSettings

        , 'TABLE_GL2DIS' =>
    'CREATE TABLE ' . TABLE_GL2DIS . ' (
    genelistid SMALLINT(5) UNSIGNED ZEROFILL NOT NULL,
    diseaseid SMALLINT(5) UNSIGNED ZEROFILL NOT NULL,
    PRIMARY KEY (genelistid, diseaseid),
    INDEX (diseaseid),
    CONSTRAINT ' . TABLE_GL2DIS . '_fk_genelistid FOREIGN KEY (genelistid) REFERENCES ' . TABLE_GENE_LISTS . ' (id) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT ' . TABLE_GL2DIS . '_fk_diseaseid FOREIGN KEY (diseaseid) REFERENCES ' . TABLE_DISEASES . ' (id) ON DELETE CASCADE ON UPDATE CASCADE)
    ' . $sSettings
          );

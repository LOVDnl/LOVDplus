        ...
        foreach ($aObjects as $nKey => $sObject) {
            switch ($sObject) {
                ...
                case 'VariantOnGenome':
                    ...
                    $aSQL['SELECT'] .= (!$aSQL['SELECT']? '' : ', ') . 'MIN(vog.id) AS vogid, MIN(vog.chromosome) AS chromosome, MIN(a.name) AS allele_' . (!$bLoadVOGEffect? '' : ', MIN(eg.name) AS vog_effect') .

...
if (LOVD_plus && PATH_COUNT == 3 && $_PE[1] == 'DBID' && !ACTION) {
    ...
    $_DATA->viewList('CustomVL_DBID', array('cols_to_skip' => 'VariantOnGenome/DBID'));
...
<?php
    ...
    $aClassificationsCount = $_DB->q($sSQLCount, array($sDBID, $sDBID))->fetchAllGroupAssoc();
...
if (PATH_COUNT == 2 && ctype_digit($_PE[1]) && in_array(ACTION, array('curate', 'edit_remarks'))) {
    ...
    $zData->setRowLink('VOT_for_curation', '');
    $aColsToShow = array('id_ncbi', 'geneid', 'VariantOnTranscript/DNA', 'VariantOnTranscript/Protein');
    $aVLOptions = array(
        'cols_to_skip' => array_diff(array_keys($zData->aColumnsViewList), $aColsToShow),
        'track_history' => false,
        'show_navigation' => false,
    );
    $zData->viewList('VOT_for_curation', $aVLOptions);
...
if (PATH_COUNT == 2 && ctype_digit($_PE[1]) && in_array(ACTION, array('confirmation_status_log', 'curation_status_log', 'curation_log'))) {
    ...
    $_DATA->viewList('Logs_for_VariantConfirmation_or_VariantCuration', $aVLOptions);

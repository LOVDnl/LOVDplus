        ...
        foreach ($zAnalyses as $key => $zAnalysis) {
            ...
            <TD class="analysis" valign="top">
              <TABLE border="0" cellpadding="0" cellspacing="1" id="' . ($zAnalysis['runid']? 'run_' . $zAnalysis['runid'] : 'analysis_' . $zAnalysis['id']) . '" class="analysis ' . $sAnalysisClassName . '" onclick="'. $sJsAction .';">
                ...
                (!$bCanRunAnalysis? '' : '
                      <IMG ' . ($bCanRemoveAnalysis? '' : 'style="display:none;"') . ' src="gfx/cross.png" alt="Remove" onclick="if(window.confirm(\'Are you sure you want to remove this analysis run? The variants will not be deleted.\')){lovd_openWindow(\'' . lovd_getInstallURL() . 'analyses/run/' . $zAnalysis['runid'] . '?delete&amp;in_window\', \'DeleteAnalysisRun\', 780, 400);} cancelParentEvent(event);" width="16" height="16" class="remove">

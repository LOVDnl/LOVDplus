...
if (!empty($_GET['id']) && $_AUTH && ACTION !== false && (isset($_SETT['curation_status'][ACTION]) || ACTION == 'clear')) {
    ...
    if ($_GET['id'] == 'selected') {
        ...
        if (!$aIDs) {
            // Nothing is selected. Die silently.
            exit;
        }
    ...
    $nSwitched = 0;
    $q = $_DB->q('UPDATE ' . TABLE_VARIANTS . ' SET curation_statusid = ' . (ACTION == 'clear'? '?' : 'CAST(? AS UNSIGNED)') . ' WHERE id IN (?' . str_repeat(', ?', count($aIDs) - 1) . ')', array_merge(array((ACTION == 'clear'? NULL : ACTION)), $aIDs), false);
    if ($q) {
